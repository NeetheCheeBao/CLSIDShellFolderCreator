...
class App(tk.Tk):
    def __init__(self):
        ...
        default_path = os.path.join(os.getcwd(), "CreatorOutput")
    ...
    def browse_dir(self):
        if self.is_creating:
            return
        dir_path = filedialog.askdirectory(initialdir=self.path_var.get(), title="选择输出目录")
        if dir_path:
    ...
    def setup_ui(self):
        ...
        path_frame.pack(side="top", fill="x", padx=15, pady=(10, 0))
        
        lbl_path = tk.Label(path_frame, text="输出路径:", bg="#FFFFFF", font=("Microsoft YaHei UI", 9))
        lbl_path.pack(side="left")
        
        self.entry_path = tk.Entry(path_frame, textvariable=self.path_var, font=("Microsoft YaHei UI", 9), 
                                   bg="#F9F9F9", relief="flat", highlightthickness=1, highlightbackground="#E0E0E0")
        self.entry_path.pack(side="left", fill="x", expand=True, padx=5)
        
        self.btn_browse = tk.Button(path_frame, text="浏览...", font=("Microsoft YaHei UI", 9), bg="#F3F3F3", 
                                    relief="flat", cursor="hand2", padx=8, command=self.browse_dir)
        self.btn_browse.pack(side="left")
        self.btn_browse.bind("<Enter>", lambda e, w=self.btn_browse: w.config(bg="#E5E5E5") if not self.is_creating else None)
        self.btn_browse.bind("<Leave>", lambda e, w=self.btn_browse: w.config(bg="#F3F3F3") if not self.is_creating else None)

        log_frame = tk.Frame(self, bg="#FFFFFF")
        log_frame.pack(side="top", fill="x", padx=15, pady=(5, 10))
        ...
        self.log_box.tag_config("yellow", foreground="#d39e00") 
        self.log_box.config(state="disabled")
    ...
    def create_all(self):
        ...
        self.btn_browse.config(state="disabled", cursor="arrow")
    ...
    def reset_create_button(self):
        ...
        self.btn_browse.config(state="normal", cursor="hand2")
